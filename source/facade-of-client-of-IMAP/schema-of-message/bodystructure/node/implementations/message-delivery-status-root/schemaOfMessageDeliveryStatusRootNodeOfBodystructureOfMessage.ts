import {MessageDeliveryStatusRootNodeOfBodystructureOfMessage} from "../../../../../../message/index.ts";
import {z} from "zod";
export const schemaOfMessageDeliveryStatusRootNodeOfBodystructureOfMessage: z.ZodType<MessageDeliveryStatusRootNodeOfBodystructureOfMessage> =
	z
		.strictObject({
			description: z.union([
				z.string().nonempty(),
		...
		.transform<MessageDeliveryStatusRootNodeOfBodystructureOfMessage>(
			function parseMessageDeliveryStatusRootNode(node) {
				if (node.disposition === null) {
					const parsedNode: MessageDeliveryStatusRootNodeOfBodystructureOfMessage =
						new MessageDeliveryStatusRootNodeOfBodystructureOfMessage(
							node.description,
							node.disposition,
							node.encoding,
							{
								parameters: {charset: node.parameters[`charset`]},
								value: node.type,
							},
						);
					return parsedNode;
