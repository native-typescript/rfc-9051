...
await describe(`UidOfFacadeOfClientOfImap`, async function executeTests(): Promise<void> {
	await test(
		`Sets flags by uid`,
		{timeout: 480000},
		async function executeTest(): Promise<void> {
			await runDockerComposeEnvironment(
				async function doWithDockerComposeEnvironment(
					...
					const subjectOfMessage = `Flags set by uid test` as const;
		...
		async function executeTest(): Promise<void> {
			await runDockerComposeEnvironment(
				async function doWithDockerComposeEnvironment(
					...
					const mailboxName = `INBOX` as const;
					const subjectOfMessage = `Flags add by uid test` as const;
					const message: rfc5322.Message = new rfc5322.WithBodyMessage(
						new rfc5322.HeadersOfMessage(
							new rfc5322.DestinationAddressFieldsOfHeadersOfMessage(
								null,
					...
					const customFlag = `Test` as const;
					await facade.uid.store.plusFlags(
						mailboxName,
						sequenceSet,
						new Set([customFlag]),
					);
		...
		async function executeTest(): Promise<void> {
			await runDockerComposeEnvironment(
				async function doWithDockerComposeEnvironment(
					...
					await facade.uid.store.minusFlags(
						mailboxName,
						sequenceSet,
						new Set([customFlag]),
					);
