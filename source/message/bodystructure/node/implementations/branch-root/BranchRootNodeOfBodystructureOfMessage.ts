		...
		>
{
	...
	public *iterateNodes(): Generator<
		...
		void
	> {
		const resultOfThis: ResultOfIteratingOfBodystructureOfMessage<this, null> =
			new ResultOfIteratingOfBodystructureOfMessage<this, null>(this, null);
		yield resultOfThis;
		for (const [index, childNode] of this.childNodes.entries()) {
			const sectionOfChildNode: SectionOfBodyOfMessage =
				new SectionOfBodyOfMessage(index + 1);
